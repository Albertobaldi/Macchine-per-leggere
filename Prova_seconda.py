...
global final_stopwords_list = st.sidebar.text_input("Inserisci una lista di stopwords, separate da una virgola (es. \"parola1, parola2, parola3\")", "").split(', ')
...
def upload ():
    uploaded_file = st.file_uploader("Carica un file .txt", type=["txt"])
    if uploaded_file is not None:
        with open(uploaded_file, "r") as f:
            text = f.read()
        return text_input(text) # return the text input
